...
tmp/.generated.sentinel: hack/bin/yq hack/bin/gci hack/bin/gofumpt hack/bin/mockery $(GO_FILES)
> mkdir -p $(@D)
...
> mkdir -p $(@D)
> go test -bench=. -benchmem -benchtime=10s -count=1 -run='^DoNotRunTests$$' -v ./...
> touch $@

tmp/.report-ran.sentinel: tmp/.generated.sentinel hack/bin/go-junit-report $(GO_FILES)
> mkdir -p $(@D)
> go test -count=1 -v ./... 2>&1 | hack/bin/go-junit-report -iocopy -out report.xml -set-exit-code
> touch $@
...
> hack/bin/golangci-lint run --fix ./...
